...
function gitinfo()
{
	...
	echo "# Description: Linux Shell script for install & setting git version control    #"
	echo "################################################################################"
	printf "\n"
}
...
{
	gitinfo
	printf "%s\n%s\n%s" "Usage: gitsetup.sh [OPTION] <args>." "You can use no option or argument. If not, you should use only one option." "Arguments depend on option."
	printf "\n%s\n" "[OPTION]"
	...
	printf "%s%s\t\t%s\n\t\t\t%s\n" "   " "<args1>" "Must be git username." "Config git username."
	printf "%s%s\t\t%s\n\t\t\t%s\n" "   " "<args2>" "Must be git email." "Config git email."
	printf "%s%s\t\t%s\n\t\t\t%s\n" "   " "<args3> " "If there are no argument(s) after, it will be timeout, else it will be timeout hour(s)." "Config git timeout for password caching."
	...
	printf "%s%s\t%s\n" "  " "gitsetup.sh -c example example@example.com 1 0 0" "Timeout is 3600 seconds. (1*3600 +0*60+ 0 = 3600)"
